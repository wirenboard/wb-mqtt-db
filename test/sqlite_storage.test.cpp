...
namespace
{
    ...
    public:
        ...
        void ProcessChannel(PChannelInfo channel) override
        {
            ...
            Fixture.Emit() << "\tLast time: "
                           << std::chrono::duration_cast<std::chrono::seconds>(channel->GetLastRecordTime() -
                                                                               std::chrono::system_clock::time_point())
                                  .count();
...
TEST_F(TSqliteStorageTest, get_averaged)
{
    ...
    storage
        ->WriteChannel(*channel, "1", "", "", true, std::chrono::system_clock::time_point() + std::chrono::seconds(5));
    storage->WriteChannel(*channel,
                          "12",
                          "10",
                          "12",
                          false,
